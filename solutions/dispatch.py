...
class Circle:
    ...
    @__init__.register(Circumference)
    def _from_circumference(self, circumference):
        self.radius = circumference.length / (2 * pi)

    @__init__.register(PointSet)
    def _from_pointset(self, pointset):
        self.radius = sqrt((pointset.outer.x - pointset.center.x)**2 + (pointset.outer.y - pointset.center.y)**2)

    @__init__.register(Point)
    def _from_point(self, point):
        self.radius = sqrt(point.x**2 + point.y**2)
